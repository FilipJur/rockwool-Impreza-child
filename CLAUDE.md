...
## Current Focus
WordPress child theme development with enterprise-grade architecture. **DEDICATED REJECT BUTTON SYSTEM COMPLETE** + **FAKTURY DOMAIN 95% IMPLEMENTED**. Successfully replaced problematic status dropdown with context-aware reject/revoke button system supporting three states: pending posts (red "Odmítnout"), published posts (orange "Zrušit schválení"), and rejected posts (grey "Odmítnuto" disabled). System eliminates validation gatekeeper conflicts and provides intuitive admin workflow. Faktury invoice domain nearly complete with dynamic points calculation (floor(value/30)), comprehensive validation service, and form integration. Only remaining: ACF field population fix and final testing. Architecture consolidated with unified Services directory reducing code duplication across domains.
...
- **2025-06-26**: [fix] INVOICE POINTS CALCULATION FORMULA - Corrected Faktury points calculation from incorrect value/10 to proper floor(value/30) formula. Updated PointsHandler to implement accurate business rule for invoice point awards. Formula now matches business requirements for invoice value to points conversion.
...
- **Faktury domain implementation** (2025-06-26): 95% COMPLETE ARCHITECTURE - Full invoice management domain with dynamic points calculation (floor(value/30)), comprehensive ValidationService with date validation, form integration, and admin interface. Complete domain architecture established: Manager, AdminController, PointsHandler, ValidationService, FakturaFieldService, FormHandler. Business rules implemented for current-year date validation. Only ACF field population fix pending for production readiness.
...
## Known Issues
- **Faktury ACF field population** (Priority: High): Points calculated correctly in dashboard but ACF editor shows empty field in post edit screen. Issue: Wrong field selector for grouped ACF fields (field_668d4f7c1a2b5 inside group field_668d4f7c1a2b4). Solution: Update FakturaFieldService field selectors to use proper group notation or add ACF load_value hook for automatic population. See `src/App/Faktury/plan.md` for detailed analysis and fix options.

*All other systems production-ready. Dedicated reject button system eliminates previous status dropdown conflicts. Consolidated services architecture provides comprehensive patterns for rapid domain implementation. English slug architecture with perfect PHP/JavaScript alignment. Template system with complete MVC separation. Ready for full deployment pending Faktury ACF field fix.*
...
## Last Updated
2025-06-26 (Dedicated reject button system implemented, Faktury domain 95% complete, consolidated services architecture, 22 commits of major architectural improvements)
